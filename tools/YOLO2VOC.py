...
#     name = child.getElementsByTagName('Class_EngName')
...
def readlabelxml(xml_path, height, width, hbb = True):
    print(xml_path)
    gt = open(xml_path).read().splitlines()
    boxes = []
    if gt:
        for img_each_label in gt:
            ...
            xx1 = str(int(center_x - bbox_width / 2))
            yy1 = str(int(center_y - bbox_height / 2))
            xx2 = str(int(center_x + bbox_width / 2))
            yy2 = str(int(center_y + bbox_height / 2))
            box = [xx1,yy1,xx2,yy2, 'ship']
            boxes.append(box)
    return boxes
...
if __name__ == '__main__':
    label_path = "/Users/liushaodong/AGraduateDesign/dataset/shipdata/val/labels"
    new_path = "/Users/liushaodong/AGraduateDesign/dataset/Ship_All/Annotations"
    img_path = "/Users/liushaodong/AGraduateDesign/dataset/shipdata/val/images"
    dst_img_path = "/Users/liushaodong/AGraduateDesign/dataset/Ship_All/JPEGImages"
    images = os.listdir(img_path)
    dst_images = os.listdir(dst_img_path)
    count = 0
    for filename in images:
        if filename in dst_images:
            continue
        name_text = filename.split('.')[0]
        label_file = label_path + '/' + name_text + '.txt'
        image_file = img_path + '/' + name_text + '.jpg'
        dst_image_file = dst_img_path + '/' + name_text + '.jpg'
        im = cv2.imread(image_file)            #读取相应的图片
        (H, W, D) = im.shape                  #返回样本的大小
        boxes = readlabelxml(label_file, H, W)
        if len(boxes) == 0:
            print('文件为空', label_file)
        writeXml(new_path, name_text, W, H, D, boxes, hbb = True)
        cv2.imwrite(dst_image_file, im)
        print('正在处理%s'%filename)
        count += 1
        with open("/Users/liushaodong/AGraduateDesign/dataset/shipdata/val/names.txt", 'a') as f:
            f.write(name_text + '\n')
    print(count)
